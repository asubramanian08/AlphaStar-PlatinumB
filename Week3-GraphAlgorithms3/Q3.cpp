...
int main(void)
{
    ...
    // BCC + count leaves of (imaginary) tree
    int compCt = BiconnectedComponents(numNodes);
    vector<int> *compGraph = new vector<int>[compCt];
    for (int i = 0; i < numNodes; i++)
        for (int next : graph[i])
            if (component[i] != component[next])
                compGraph[component[i]].push_back(component[next]);
    int leaves = 0;
    for (int i = 0; i < compCt; i++)
    {
        sort(compGraph[i].begin(), compGraph[i].end());
        leaves += (compGraph[i].size() && compGraph[i].front() == compGraph[i].back());
    }
